...
lib.getUser = async (req,res,next) => {
		...
		if(id){
		let user = await User.findById(id).catch( ()=> console.log('failed to get user') );
...
lib.getUsers = async (req,res,next) => {
		//Required data
		let users = await User.find({}).catch( ()=> console.log('failed to get user') );
...
lib.updateUser = async (req,res,next) => {
			const userDetails = req.body
			if(!userDetails) return res.status(400).send({error : 'Missing fields to update'})
			const updateFields  = _.omitBy(userDetails,_.isEmpty)
			if(Object.keys(userDetails).length === 0 ) return res.status(400).send({error : 'Missing fields to update'})
			console.log("updateFields",updateFields)
			//save the new user date
			let user = await User.findByIdAndUpdate(userDetails.id,updateFields,{new:true});
			if(!user) return res.sendStatus(404);
			delete user.password;
			return res.status(200).send(user);
...
lib.loginUser = async (req,res) => {
	...
	if(valid.error) return res.status(400).send({error:'Missing required fields'});
	//lookup the user
	let user = await User.findOne({email:credentials.email}).catch( () => console.log('failed to find user'));
